...
class MinMaxAgent:
    ...
    def decide(self, connect4: Connect4):
        ...
        action, value = self._minimax(deepcopy(connect4), self.max_depth, True)
        return action

    def _minimax(self, connect4: Connect4, depth, is_maximizing_player):
        pos = None
        ...
        if is_maximizing_player:
            ...
            for position in connect4.possible_drops():
                new_connect4 = deepcopy(connect4)
                new_connect4.drop_token(position)
                _, new_val = self._minimax(new_connect4, depth - 1, True)
                if new_val < val:
                    pos, val = position, new_val
